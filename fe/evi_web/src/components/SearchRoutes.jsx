...
const SearchRoutes = () => {
  ...
  const filteredSchedules = schedules.filter((schedule) => {
    const searchValue = `${schedule.route.startLocation} ${schedule.route.endLocation} ${schedule.vehicle.vehicleNumber} ${schedule.vehicle.vehicleType}`.toLowerCase();
    const matchesSearch = searchValue.includes(searchTerm.toLowerCase());
    
    const matchesTimeRange = !filters.timeRange || isInTimeRange(schedule.departureTime, filters.timeRange);
    const matchesVehicleType = !filters.vehicleType || schedule.vehicle.vehicleType === filters.vehicleType;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await searchSchedules(origin.value, destination.value, travelDate);
      const filteredData = filterCurrentAndFutureSchedules(data);
      setSchedules(filteredData);
    ...
    <div className="container">
              ...
              <div className="card">
                <div className="card-body">
                  <div className="trip-type-toggle mb-3 d-flex align-items-center">
                    <Button variant="link" className="card-title ml-auto" onClick={() => setShowModal(true)}>
                      Hướng dẫn mua vé
                    </Button>
                  </div>
                    ...
                    <div className="form-group">
                      <div className="row">
                        <div className="col-md-5">
                          <label htmlFor="origin">Điểm đi <span className="text-danger">*</span></label>
                          <Select
                            id="origin"
                            options={provinces}
                            value={origin}
                        ...
                        <div className="col-md-5">
                          <label htmlFor="destination">Điểm đến <span className="text-danger">*</span></label>
                          <Select
                            id="destination"
                            options={provinces}
                            value={destination}
                    ...
                    <div className="form-group">
                      <div className="row">
                        <div className="col-md-6">
                          <label htmlFor="travelDate">Ngày đi <span className="text-danger">*</span></label>
                          <input
                            type="date"
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        ...
        <Modal.Body>
          ...
          <p>3. Sử dụng các bộ lọc để tìm kiếm chuyến xe theo thời gian, loại xe, và giá vé.</p>
            ...
            <div className="card-body">
              ...
              <div className="form-group">
                ...
                <Select
                  id="timeRange"
                  options={timeRanges}
                  value={timeRanges.find((range) => range.value === filters.timeRange)}
                  onChange={(selectedOption) => setFilters({ ...filters, timeRange: selectedOption?.value || '' })}
                  isClearable
                />
              </div>
              <div className="form-group">
                ...
                <Select
                  ...
                  value={vehicleTypes.find((type) => type.value === filters.vehicleType)}
                  onChange={(selectedOption) => setFilters({ ...filters, vehicleType: selectedOption?.value || '' })}
                  isClearable
                />
              </div>
              <div className="form-group">
                ...
                <Select
                  ...
                  onChange={(selectedOption) => setFilters({ ...filters, sortBy: selectedOption?.value || '' })}
                  isClearable
                />
              </div>
            </div>
          ...
          <div className="card">
            ...
            <div className="card-body">
                ...
                <tbody>
                  {filteredSchedules.map((schedule) => (
                    <tr key={schedule.scheduleId}>
                      <td>{schedule.route.startLocation} - {schedule.route.endLocation}</td>
                      <td>{moment(schedule.departureTime).format('DD/MM/YYYY')}</td>
                      <td>{schedule.vehicle.vehicleNumber}</td>
                      <td>{schedule.vehicle.vehicleType}</td>
                      <td>{formatTime(schedule.departureTime)} <RiArrowRightDoubleLine /> {formatTime(schedule.arrivalTime)}: ({calculateDuration(schedule.departureTime, schedule.arrivalTime)})</td>
              ...
              {filteredSchedules.length === 0 && !error && (
                <div className="alert alert-info">Không có chuyến xe nào phù hợp.</div>
              )}
            </div>
          </div>
        </div>
