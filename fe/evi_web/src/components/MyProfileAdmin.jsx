...
import { changePasswordAdmin, updateProfileAdmin } from '../services/UserService';
...
function MyProfile() {
  ...
  const handleSaveChange = async (event) => {
    ...
    try {
      const response = await updateProfileAdmin(user.id, event.target.avatar.files[0], fullname, email, phoneNumber, address);
      ...
      if (currentPassword === "" || confirmPassword === "" || newPassword === "") {
        ...
        if (currentPassword === user.password) {
          if (newPassword === confirmPassword || newPassword === "") {
            await changePasswordAdmin(user.id, newPassword);
