...
const Schedules = () => {
  ...
  const bookedSeats = bookings.flatMap((booking) =>
    booking.bookingDetails.map((detail) => detail.seat.seatNumber)
  );
...
const TripInfo = ({
  ...
  const departureHour = new Date(departureTime).toLocaleTimeString("vi-VN", {
    hour: "2-digit",
    minute: "2-digit",
  });
  ...
  return (
    <div className="card mb-4">
      ...
      <div className="card-body">
        <InfoItem
          icon={FaMapMarkerAlt}
          text={`Tuyến xe: ${route.startLocation} → ${route.endLocation}`}
        />
        ...
        {selectedSeats.length > 0 && (
          <InfoItem
            icon={FaInfoCircle}
            text={`Bạn đã chọn các ghế: ${selectedSeats.join(", ")}`}
          />
        )}
        <h4 type="text" className=" mb-3">
          Chi tiết giá
        </h4>
  ...
  const SeatRow = ({ seats }) => (
    <tr>
      {seats.map((seat, index) => (
        ...
        >
          <PiSeatLight
            className={selectedSeats.includes(seat) ? "text-primary" : ""}
          />
        ...
        <div className="row">
          <div className="col-6">
            ...
            <table className="table table-bordered">
              <tbody>
                {lowerDeckSeats.map((seats, index) => (
          ...
          <div className="col-6">
            ...
            <table className="table table-bordered">
              <tbody>
                {upperDeckSeats.map((seats, index) => (
                  <SeatRow key={index} seats={seats} />
                ))}
              </tbody>
            </table>
...
const CustomerInfoForm = ({
  ...
  const filteredDropoffStations = stationOptions.filter(
    (option) => !pickupStation || option.stopOrder > pickupStation.stopOrder
  );
...
const ImportantNotes = () => (
  <div className="card">
    ...
    <div className="card-body">
      <p>
        1. Quý khách vui lòng có mặt tại điểm đón trước giờ khởi hành ít nhất 15
        phút.
      </p>
